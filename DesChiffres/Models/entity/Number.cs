...
namespace DesChiffres.Models.entity
{
    public class Number
    {
        ...
        List<int> bestClosest = new List<int>();
        List<string> bestExpression = new List<string>();

        public List<int> BestClosest { get => bestClosest; set => bestClosest = value; }
        public List<string> BestExpression { get => bestExpression; set => bestExpression = value; }
        ...
        public bool verif_closest(int next_closest)
        {
            int [] list = this.BestClosest.ToArray();
            for (int i = 0; i < list.Length; i++)
            {
                if (list[i] == next_closest)
                {
                    return true;
                }
            }
        ...
        public bool verif_expression (string next_expression)
        {
            string[] list = this.BestExpression.ToArray();
            for(int i=0; i < list.Length; i++)
            {
                if (string.Compare(list[i], next_expression) == 0)
                {
                    return true;
                }
            }
        ...
        public int FindBestClosest(int[] numbers, int targetNumber)
        {
            int bestResult = int.MaxValue;
            string bestExpression = "";
            ...
            for (int i = 0; i < numbers.Length; i++)
            {
                ...
                bool top = false;
                FindBestCombinationRecursive(numbers, targetNumber, numbers[i], numbers[i].ToString(), ref bestResult, ref bestExpression, used);
                if (!this.verif_closest(bestResult))
                {
                    this.BestClosest.Add(bestResult);
                    this.BestExpression.Add(bestExpression);
                    top = true;
                }
                if (this.verif_closest(bestResult) && top==false)
                {
                    if (!this.verif_expression(bestExpression))
                    {
                        this.BestClosest.Add(bestResult);
                        this.BestExpression.Add(bestExpression);
                        top = true;
                    }
                    if (this.verif_expression(bestExpression) && top==false)
                    {
