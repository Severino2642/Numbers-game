var baseUrl = "https://localhost:7176/api";
...
app.controller("MainController", function ($scope, $http, $interval) {
    ...
    function getClosestNumber() {
        $http.get(`${baseUrl}/game/getClosestNumber/`, { params: { guessNumber: $scope.Number, listNumber: JSON.stringify($scope.listNumber) } }).then(function (response) {
            $scope.playerNumber = response.data.listNumber;
            $scope.listCombinaison = response.data.listExpression;
            console.log("AI Number : ", response.data.listExpression);
            addSuggestion();
        }).catch(function (error) {
            console.error("Error : ", error);
        });
    ...
    function addSuggestion() {
        let compt = 0;
        for (let i = $scope.playerNumber.length - 1; i >= $scope.playerNumber.length - 4; i--) {
            $scope.listPlayer[compt].number = $scope.playerNumber[i];
            // $scope.validation += "" + $scope.listPlayer[0].id + "_" + $scope.listPlayer[0].number;
            document.querySelector("." + $scope.listPlayer[compt].name).value = $scope.playerNumber[i];
            document.querySelector(".comb-" + $scope.listPlayer[compt].name).textContent = $scope.listPlayer[compt].name + " : " + $scope.listCombinaison[i];
    ...
    function sendResult() {
        ...
        if ($scope.validation != "") {
            ...
            $scope.gameResult = "Aucun joueur n'a envoyer ou ecrit une suggestion";
        }
    }
    ...
    $scope.sendCombinaison = function () {
        ...
        $http.get(`${baseUrl}/game/verifCombinaison/`, { params: { combinaison: $scope.combination, listNumber: JSON.stringify($scope.listNumber) } }).then(function (response) {
            ...
            if (reponse_verification == 0) {
                $scope.gameResult = "Il y a une erreur dans la combinaison!";
            }
    ...
    function verdicte() {
        ...
        if ($scope.valiny == $scope.listPlayer[0].number) {
            $scope.gameResult = $scope.listPlayer[0].name + " a gagner car le resultat de la combinaison :  " + $scope.valiny + " = " + $scope.listPlayer[0].number;
        }
        else {
            if ($scope.listPlayer.length >= 2) {
                $scope.gameResult = $scope.listPlayer[1].name + "a gagner car le resultat de la combinaison : " + $scope.valiny + " != " + $scope.listPlayer[0].number;
            }
            if ($scope.listPlayer.length < 2) {
                $scope.gameResult = "Le joueur adverse a gagner car le resultat de la combinaison : " + $scope.valiny + " != " + $scope.listPlayer[0].number;
            }
        }
    }
